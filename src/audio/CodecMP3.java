...
public class CodecMP3 extends Codec
{
	...
	{
		length -= length % (sampleSize * channels);
		...
		return new AudioBuffer(streamBuffer, bytesRead, samplerate, alFormat);
	}
	...
	public AudioBuffer readAll()
	{
		...
		if (fileSize > 0)
		{
			fullBuffer = new byte[Math.min(fileSize, AudioSystem.defaultSoundBufferSize)];
			try
			{
		...
		{
			fullBuffer = new byte[AudioSystem.defaultSoundBufferSize];
		...
		return new AudioBuffer(fullBuffer, bytesRead, samplerate, alFormat);
	}
